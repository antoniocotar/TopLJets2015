...
class Jet : public TLorentzVector {
  public:
    Jet(TLorentzVector p4, int flavor, int idx)
      : TLorentzVector(p4), deepjet_(0), pumva_(0), flavor_(flavor), idx_(idx), overlap_(0), scaleUnc_(0) {}

    Jet(TLorentzVector p4, float deepjet, int idx)
      : TLorentzVector(p4), deepjet_(deepjet), pumva_(0), flavor_(0), idx_(idx), overlap_(0), scaleUnc_(0) {}

    Jet(const Jet &j)
      : TLorentzVector(j.Px(), j.Py(), j.Pz(), j.E()), particles_(j.particles_), trks_(j.trks_), deepjet_(j.deepjet_), pumva_(j.pumva_), flavor_(j.flavor_), idx_(j.idx_), overlap_(j.overlap_), scaleUnc_(j.scaleUnc_) {}
    ...
    static bool sortJetsByDeepJET(Jet i, Jet j) { return i.getDeepJET() > j.getDeepJET(); }
  ...
  private:
    ...
    float deepjet_; // New member for DeepJet
    float pumva_;
    int flavor_;
    int idx_;
    int overlap_;
